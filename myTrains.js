...
function ShowRealTimeData (res) {
    ...
    and stopTimes.stop_id like '${stop2}%';`;
	//console.log(script);
	db.serialize(() => {
		db.run("drop table if exists trip1").
			run(script);
		db.all(`select trip1.trip_id,stop1,stop2,stop1_arrival,stop2_arrival,arrival_delay  from trip1 join trip2 on trip1.trip_id=trip2.trip_id  
        ...
        order by stop1_arrival;`, (err, rows) => {
			if (err){
				throw err;
			}
			rows.forEach( row => {
				...
				var a1=moment(row.stop1_arrival, "HH-mm-ss");
				console.log(a1.add(row.arrival_delay, "seconds"));
				console.log(minutes);
...
app.get("/mytrains", function (req, res) {
